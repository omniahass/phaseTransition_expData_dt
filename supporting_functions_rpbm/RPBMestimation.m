classdef RPBMestimation
    ...
    methods     ( Static = true )
        ...
        % Setting D0 to default values if not an input
        ...
        if isscalar(D0)
            ...
            D0_test = RPBMestimation.vectorize(D0, mask);
            D0_train = mean(D0_test);
       end

        t = t(:)';
        bounds(1,:) = [ D0_train D0_train ];
        [D0_tr,a_tr,kappa_tr,tau_tr,zeta_tr] = RPBMestimation.sample_rpbm_param(bounds,Ntrain,diam_factor);
        ...
        Dt_train = D0_tr.*RPBMestimation.get_Dt_RPBM_vectorized(tnorm_all, zeta_all);
        ...
        options.training_data = options.training_data./D0_train;
        options.training_objective = [zeta_tr(:) kappa_tr(:) a_tr(:)]';
        options.test_data = Dperp./D0_test;
        out = RPBMestimation.DataDriven_regression(options);
