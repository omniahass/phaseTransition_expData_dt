...
output_folder = fullfile(parent_directory,'rpbm_maps_1p25sigma');
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end
residuals_folder = '/Users/gabriellebaxter/Documents/HEAL_Volunteers/cornell/resid_estimates';
if ~exist(residuals_folder, 'dir')
    mkdir(residuals_folder);
end
...
for i = 1:length(uniqueNames)
    ...
    if exist(filename,'file')
        ...
        for t = 1:nT
            if t == 1
                filename = fullfile(maps_folder, ...
                    sprintf('%s_%s_AD.nii.gz', subject, times{t}));
            else
                filename = fullfile(maps_folder, ...
                    sprintf('%s_%s_AD_withinscanreg.nii.gz', subject, times{t}));
            end
            nii = load_untouch_nii(filename);
            vol = nii.img;
            if t == 1
        ...
        % 
        filename = fullfile(residuals_folder, sprintf('%s_sigma_dperp.nii', subject));
        ...
        %     ti = t(i);
        % 
        %     Dt_norm = RPBMestimation.get_Dt_RPBM_vectorized( ...
        %                 ti * ones(Nvox,1), zeta_v);
        % 
        %     RD(:,i) = D0_v .* Dt_norm;
        % end
        % 
        % RD = reshape(RD, X, Y, Z, Nt);
        % 
        % residual = Dperp - RD;          % [X,Y,Z,Nt]
        % res_std  = std(residual, 0, 4);
        % filename = fullfile(residuals_folder, sprintf('%s_residuals_std.nii', subject));
        % niftiwrite(res_std,filename,info);
