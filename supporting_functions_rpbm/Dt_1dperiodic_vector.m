...
zeta = D0/(kappa*a);
td = a^2/D0;
tbar = t(:)'/td;    % force row vector

M = 40;

beta = arrayfun(@(m) fzero(@(k) tan(k/2)+zeta*k/2, [(2*m-1)*pi+1e-12, 2*m*pi]), (1:M)');

% M x N matrix
E = exp(-(beta.^2) .* tbar);

weights = 1 ./ ...
    (beta.^2 .* (1 + zeta + (zeta*beta/2).^2));

Dt = D0 * ( ...
    1/(1+zeta) + ...
    2*zeta^2 ./ tbar .* sum((1 - E).*weights,1) ...
    );

Dt = Dt(:);   % return column vector for fit()
